...
shap_day_local <-  readRDS(file.path(path_models,
                                       "imp_shap_grouped_all_1day_0_v4.rds")) shap_hour_local <-  readRDS(file.path(path_models,
...
## Local Variable Importance by Model{.medium}
...
# sample full observations (ID)
id_max <- shap_week_local %>% pull(ID) %>% max
set.seed(1234)
ids <- sample(1:id_max, 20000)

shap_week_local %>%
  ungroup() %>% 
  filter(ID %in% ids) %>% 
  # mutate(group = reorder(group, mean_value, sum)) %>%
  ggplot(aes(x = shap, y = group)) +
  ggforce::geom_sina(binwidth = .2, maxwidth = .5, scale = "width")
  #ylab("SHAP value (impact on lapse probability)") +
  #xlab("") +
  #coord_flip() 
